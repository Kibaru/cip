...
export class ScanBoardingPassComponent implements IModalDialog {
  ...
  airlines: any[] = [];
  services: any = [];
  payments: any = [{"name":"Cash"}, {"name":"Credit"}];
  ...
  ngOnInit(): void {
    ...
    document.addEventListener('keypress', e => {
      ...
      if (e.keyCode === 13) {
        ...
        // code ready to use 
        // this.onCodeResult(this.qrResultString);               
        code = "";
      } else {
  ...
  saveNewEntry() {
    ...
    //   return;
    // };
    if (this.airlines.filter(a => a.iata_code == this.airline).length <= 0){
      // console.log(this.airline);
      this.notifier.notify('error', 'The Airline Of the guest is Required. Please confirm with system administrator that this airline is in your list of supported airlines.');
      return;
    }
    ...
    if (this.is_redeemable) {
      ...
      // console.log(entry_payload); return;

      this.ngxService.start();
      this.api.saveNewEntry(entry_payload)
        .subscribe((save_entry_data: any) => {
          // console.log(save_entry_data); return;
          if (save_entry_data.hasOwnProperty("error")) {
            this.notifier.notify('error', save_entry_data.error);
            this.ngxService.stop();
            return;
