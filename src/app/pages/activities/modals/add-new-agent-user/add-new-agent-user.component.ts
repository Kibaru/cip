...
export class AddNewAgentUserComponent implements IModalDialog {
  ...
  roles: any = [{"role":"admin_agent"}, {"role":"user_agent"}];
  is_travel_agent : boolean = false;
  ...
  ngOnInit(): void {
    console.log(this.dataStore.userInfo.getValue());
    
    if(this.dataStore.userInfo.getValue().is_travel_agent == 1){
      this.is_travel_agent = true;
    }
  }
